...
public class Matrix {
    ...
    private final VectorType orientation;
    
    private final Vector[] space;
    
    public Matrix(final VectorType orientation, final int rows, final int cols) {
        this(orientation, new double[rows][cols]);
    }
    
    //Major X Minor
    public Matrix(final VectorType orientation, final double[][] rawDat) {
    ...
    public final Matrix flip() {
        ...
        switch(orientation) {
            ...
            case ROW:
                tranny = new Matrix(VectorType.COLUMN, space[0].size(), space.length);
                break;
            case COLUMN:
                tranny = new Matrix(VectorType.ROW, space.length, space[0].size());
                break;
        }
    ...
    public final Matrix process(final Matrix inputField) {
        //if(this.orientation != VectorType.COLUMN) {
        //    throw new UnsupportedOperationException("Weight Matrix orientation incorrect");
        //}
        if(inputField.orientation() != VectorType.ROW) {
        ...
        return new Matrix(VectorType.ROW, rawOutput);
    }
    ...
    public final VectorType orientation() {
